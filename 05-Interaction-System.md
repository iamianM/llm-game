...
Each conversation is **3-5 exchanges maximum**, then ends:
...
// Exchange 1
playerSelectsOption("flirt")
→ generateDialogue()
→ displayResult()
conversation.exchanges.push({ type: "flirt", success: true, time: 20 })
conversation.totalTime += 20

// Player chooses to continue
playerSelectsOption("deep_conversation")
→ generateDialogue()
→ displayResult()
conversation.exchanges.push({ type: "deep", success: true, time: 25 })
conversation.totalTime += 25
...
### Why 3-5 Exchange Limit?

**Prevents:**
- ❌ Player camping on one person all day
- ❌ Conversations feeling too long
- ❌ Time management becoming meaningless

**Encourages:**
- ✅ Spreading interactions across multiple Islanders
- ✅ Strategic choice (who needs attention most?)
- ✅ Coming back to people later (build over time)
...
→ Keep talking (~20-30 min)
   • Deep conversation
   • Flirt more
   • Ask about villa drama

→ Suggest activity together (~30-45 min)
   • Invite to swim
   • Work out together
   • Private chat on terrace

→ End conversation (save time)
```

**Strategic decision:**
- Continue = deepen connection, but use time
- End = preserve time for others, but miss opportunity
