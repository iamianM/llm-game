...
def test_couple_ranking_orders_by_strength_then_perception() -> None:
    state = new_game(1)
    state.heartbreakers[0].relationship.affection = 90
    state.heartbreakers[0].relationship.trust = 70
    state.couples = [
...
def test_steal_chance_includes_chemistry_minus_couple_strength() -> None:
    state = new_game(1)
    heart_throb = state.heartbreakers[1]
    heart_throb.archetype = "heart_throb"
    heart_throb.relationship.chemistry = 5
    couple = Couple(partner_a_id="player", partner_b_id="chloe", formed_on_day=1)

    assert steal_chance(state, heart_throb, "chloe", couple) == 70
...
def test_steal_chance_clamped_to_10_90() -> None:
    ...
    heart_throb = state.heartbreakers[1]
    couple = Couple(partner_a_id="player", partner_b_id="chloe", formed_on_day=1)
    heart_throb.relationship.chemistry = 100

    assert steal_chance(state, heart_throb, "chloe", couple) == 90
...
def test_steal_success_swaps_partners() -> None:
    ...
    attempt = resolve_steal_attempt(state, heart_throb.id, state.couples[0], SeededRng(1))

    assert attempt.success is True
    assert state.couples[0].partner_a_id == heart_throb.id
...
def test_steal_failure_keeps_couple_intact() -> None:
    ...
    state.couples = [Couple(partner_a_id="player", partner_b_id="chloe", formed_on_day=1)]
    state.heartbreakers[0].relationship.affection = 90
    state.heartbreakers[0].relationship.trust = 90
    heart_throb = state.heartbreakers[1]
    heart_throb.relationship.chemistry = 0
