...
from src.game.engine.arrival_rolls import interruption_chance, pull_chance, roll_arrival
...
def test_pull_chance_subtracts_player_couple_strength() -> None:
    state = new_game(1)
    ...
    assert pull_chance(state, maya, "chloe") < 20


def test_pull_chance_clamped() -> None:
    state = new_game(1)
    maya = state.islanders[1]
    maya.relationship = RelationshipState(chemistry=100)

    assert pull_chance(state, maya, "chloe") == 60
