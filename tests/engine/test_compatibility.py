...
from src.game.state.models import AttachmentStyle, Big5, IdealMatch, new_game
...
def test_compatibility_bonus_increases_with_matching_values() -> None:
    ...
    assert compatibility_bonus(state, state.heartbreakers[0], ["loyalty"]) > 0


def test_compatibility_bonus_capped_at_20() -> None:
    state = new_game(1)
    target = state.heartbreakers[0]
    target.ideal_match.personality_type = ["a", "b", "c", "d", "e", "f"]
...
def test_dealbreaker_penalty_applied_when_player_carries_tag() -> None:
    ...
    assert dealbreaker_penalty(state.heartbreakers[0], ["arrogance"]) == 15


def test_dealbreaker_penalty_not_double_counted() -> None:
    state = new_game(1)

    assert dealbreaker_penalty(state.heartbreakers[0], ["arrogance", "arrogance"]) == 15


def test_attachment_secure_deep_success_bonus() -> None:
    state = new_game(1)

    delta = attachment_delta_modifier(state.heartbreakers[0], "honest_vulnerable", True)
...
def test_attachment_anxious_amplifies_miss_trust_loss() -> None:
    ...
    delta = attachment_delta_modifier(state.heartbreakers[1], "escalate_flirt", False)
...
def test_ideal_match_required_fields() -> None:
    prefs = IdealMatch(
