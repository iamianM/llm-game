"""Tests for Heartbreaker Voice native message-chain construction."""
...
def test_build_voice_messages_first_exchange_has_scene_and_turn() -> None:
    ...
    messages = build_voice_messages(
        state,
        conversation,
        new_turn_context(heartbreaker_voice_context(state, result)),
    )
...
def test_build_voice_messages_guards_against_cross_conversation_openers() -> None:
    # A fresh conversation has no prior exchanges of its own, but the resort-wide
    # recent-player-lines buffer carries the opener used with the last heartbreaker.
