...
Phase G shipped a living resort with conversations, memories, and gossip. G7 surfaced enough of that for the user to actually play and review. G8 fixes the four things the first real playthrough exposed:
...
3. **Private Chat is unconditional.** `Talk to Chloe` always starts a conversation even when Chloe is mid-flirt with Liam. The Pull System from [09-Social-Dynamics.md](../09-Social-Dynamics.md) is absent.
...
|---|---|---|---|---|
| **Graceful wheel exit** | RESPOND_WITH with `option.category == "exit"` OR `intent_kind in EXIT_INTENT_KINDS` | Yes — Heartbreaker Voice writes a goodbye exchange | Yes | `trust +1` for ending warmly |
...
Lives in `engine/private_chat.py`. Triggered automatically when player picks `START_CONVERSATION` and the target is in an active `NPCNPCConversation`. If the target is alone, no pull roll — conversation starts normally.

Pull chance formula:

```
chance = 50 + (spark × 4) + (player_affection_with_target ÷ 4)
...
Where:
- `spark` is `state.player.stats.spark`
- `player_affection_with_target` is `state.heartbreakers[target].relationship.affection`
...
On miss: no conversation starts. Engine generates a brief deflection exchange via Heartbreaker Voice with `intent_kind="pull_rejected"`. Small `affection -1` to the target (you imposed). Bystanders at the location form a witnessed memory ("X tried to pull Y away from Z and got brushed off").
...
`NPCInterruption` Pydantic model added to `ResortUpdate`:
...
The Resort Orchestrator decides when to fire one (prompt update). At most one per turn. Cannot fire if the player has no active conversation. Cannot fire if `state.active_conversation.pending_interruption is not None` (one at a time).
...
- "At least one START_CONVERSATION triggered a private chat attempt."
- "At least one private chat attempt failed."
...
- **Mini resort map per turn:** small left-rail summary of "who's where" at the start of the turn.
...
- **Color-coded event types:** success greens, miss reds, private chat attempts oranges, interruptions purples.
...
- `src/game/engine/rules.py`:
  ...
  - In the RESPOND_WITH handling: when the chosen option's `category == "exit"` OR `intent_kind in EXIT_INTENT_KINDS`, after applying deltas, generate the exchange via Heartbreaker Voice (the normal path), then close the conversation. Run Curator. Apply `trust +1` to the target (graceful exit bonus).
  - END_CONVERSATION path: apply `affection -1` to the active conversation's target before closing. Curator still runs.
- `src/game/engine/turn.py`: update the conversation-close path to handle the new "wheel-exit triggers close" flow. Make sure Curator and close happen after Heartbreaker Voice generates the exchange, not before (otherwise the goodbye exchange doesn't get into the curated memory batch).
...
- No changes to Private Chat or interruption logic (those are G8.2 and G8.3).
- No changes to the wheel rendering or prompt files (G8 doesn't touch prompts).

---

## Phase G8.2 — Private Chat
...
- `src/game/engine/private_chat.py` (new):
...
- `src/game/cli/commands/play.py`: when a private chat attempt is recorded, the turn output shows it explicitly: `"You tried to pull Chloe (62% chance) — she demurred."` followed by the deflection exchange if miss.
- `src/game/reporting/html.py`: private chat attempts get their own card style (orange).

**Tests (engine, non-LLM).**

- `tests/engine/test_private_chat.py` (new):
  - `test_private_chat_chance_higher_with_more_spark` — spark 9 gives higher chance than spark 3 at same affection.
  - `test_private_chat_chance_lower_when_target_chemistry_strong` — high target chemistry reduces chance.
  - `test_private_chat_chance_privacy_modifier_applied` — bedroom > pool > kitchen.
  - `test_private_chat_chance_clamped_to_10_90` — extreme inputs respect the clamp.
  - `test_target_in_active_conversation_returns_correct_conv` — utility correctness.
  - `test_private_chat_succeeds_when_target_alone_no_roll_needed` — START_CONVERSATION on a target not in any NPC convo skips the pull entirely.
- `tests/engine/test_rules.py`:
  ...
  - `test_private_chat_failure_applies_affection_penalty` — target loses 1 affection.
  - `test_private_chat_failure_bystanders_get_witness_memory` — bystanders at the location form witnessed memories tagged `saw_pull_rejected`.
  - `test_private_chat_success_closes_targets_existing_conv` — when pull succeeds, the target's old convo closes with reason `pulled_away`.
  - `test_private_chat_attempt_recorded_in_mechanical_result` — the `pull_attempt` field is populated on START_CONVERSATION when there was a contested target.

**Tests (LLM, opt-in `-m llm`).**

- `tests/agents/test_heartbreaker_voice_private_chat_rejected.py`:
  - Parametrized over the four NPCs (Chloe, Maya, Liam, Aisha heart_throb).
...
- No prompt changes — Heartbreaker Voice already handles ad-hoc `intent_kind` values.
...
**Scope.** The Resort Orchestrator can fire an NPC interruption during the player's active conversation. When it fires, the player's next wheel shows three special options (Welcome / Defer / Ignore) at the top with appropriate flavor. Each option has its own mechanical outcome and memory effects.
...
- `src/game/agents/resort_orchestrator.py`:
  - Extend `ResortUpdate` to include `npc_interruptions: list[NPCInterruption] = []` (max 1 element per turn enforced by validator).
  - Update the prompt at `src/game/agents/prompts/resort_orchestrator.md` (Claude rewrites — install verbatim).
- `src/game/engine/resort.py`:
  - `validate_resort_update` rejects: more than one interruption per turn, interruption when player has no active conversation, interruption when one is already pending, interruption where interrupter_id is at a different location than the player.
  - `apply_resort_update` writes the interruption to `state.active_conversation.pending_interruption`.
...
- `src/game/engine/rules.py`:
  ...
  - Add `HeartbreakerState.tags_to_remember` mechanism: interruptions where the player handled them poorly (`snubbed_publicly`, `ignored_in_public`) become memories with weight 7-8 (very gossip-worthy).
...
- `tests/engine/test_interruptions.py` (new):
  - `test_orchestrator_can_emit_interruption_in_resort_update` — ResortUpdate schema accepts NPCInterruption.
  - `test_resort_update_rejects_two_interruptions_in_one_turn`.
  - `test_resort_update_rejects_interruption_when_no_player_conv`.
  - `test_resort_update_rejects_interruption_when_one_already_pending`.
  - `test_resort_update_rejects_interruption_at_wrong_location`.
...
- `tests/agents/test_resort_orchestrator_interruptions.py`:
...
- Initial assertion suite (each in its own pure function):
  ...
  - `assert_ceremony_event_observed` — Pairing Ceremony or heart_throb event fired.
...
- `src/game/reporting/html.py`:
  ...
  - Per-turn card gains a **"Resort snapshot" subsection** showing the pre-turn resort map (4 locations × who's where) and active NPC convos with topic.
...
- `tests/cli/test_report.py`:
  ...
  - `test_session_html_includes_resort_snapshot`.
...
- `make qa` green.
- Opening `review-packet/session.html` from a real playthrough shows every turn with the success math visible, Sunset Bay snapshot, any memories formed, any private chat attempts, any interruptions, color-coded.
...
The resort orchestrator prompt gets one additive update for interruption support. I (Claude) own the rewrite per R17 — Codex installs verbatim.

- [`src/game/agents/prompts/resort_orchestrator.md`](../src/game/agents/prompts/resort_orchestrator.md) — **G8.3.** I add an `## Interruptions` section to the prompt describing the new `npc_interruptions` output field, when to fire, and the four reason/urgency vocabularies. Existing sections unchanged.

No other prompt changes in G8. Pull rejection reuses Heartbreaker Voice with `intent_kind="pull_rejected"` — the existing prompt's fallback handling for unknown intent_kinds covers it.
...
- ❌ No new agents beyond what G shipped. G8 reuses Heartbreaker Voice, Contextual Options, Curator, Resort Orchestrator, Background Dialogue.
- ❌ No prompt rewrites except the one resort_orchestrator update I own.
...
4. `make play` exercises all four new mechanics live: initial intents respect risk; wheel exits actually exit; private-chat fires with a roll when target is busy; interruptions appear with three options.
