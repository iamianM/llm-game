...
## 11. Implemented minigame families

The shared harness and presentation registry cover all six scheduled families:

1. Compatibility Quiz uses knowledge-backed answer rounds.
2. Pulse Race exposes recorded performer and observer readings.
3. The Couples Quiz compares the player's answer with the partner reveal.
4. Lie Detector exposes the engine verdict and recorded roll.
5. Kiss Wed Pass records a constrained three-role allocation.
6. Final Couples scores five facets and records the final tally.

Per-kind mechanics remain in the owning specs under `docs/minigames/`. The
shared presentation contract stays deliberately smaller than those mechanics:
it carries only truthful display values and never legal choices.
