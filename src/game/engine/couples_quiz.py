"""The Couples Quiz (couples_quiz) minigame.
...
def build_rounds(state: GameState, partner_id: str, rng: SeededRng) -> list[MinigameRound]:
    ...
    for index in range(ROUNDS):
        if index % 2 == 0:
            ...
            peer_genders = {i.id: (i.gender.value if i.gender else None) for i in state.heartbreakers}
            distractors = [
                (_neutralize_for_distractor(d, target_gender) or d) for d in prompt.distractors
            ]
            # Pad distractors from same-key cross-heartbreaker values, preferring
            ...
            if len(distractors) < 3:
                ...
                for o in others:
                    ...
                    if len(distractors) >= 3:
                        break
            round_rng = rng.fork(f"couples_quiz::round::{index}")
            ...
            if index == 0:
                scene = (
                    ...
                    "booth on one side; you're at the flame_deck with a clipboard. The "
            ...
            _shuffle(choices, round_rng)
            stem_question = {
                "archetype": f"In the booth, {partner.name} was asked what opening vibe you came in as — what did they write down?",
...
def submit_choice(challenge: Challenge, choice_id: str) -> Challenge:
    if challenge.classification is not None:
        raise ValueError("couples_quiz already resolved")
...
def score_couples_quiz(state: GameState, challenge: Challenge) -> Challenge:
    bal = load_minigame_balance().couples_quiz
...
def apply_couples_quiz_result(state: GameState, challenge: Challenge) -> Challenge:
    ...
    for r in challenge.rounds:
        ...
        state.player.known_facts[fact_key] = KnownFact(
            ...
            citation=f"couples_quiz day {state.day}",
        )
