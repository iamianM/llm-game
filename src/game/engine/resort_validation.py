"""Validation helpers for Resort Orchestrator commits."""

from __future__ import annotations

from src.game.agents.resort_orchestrator import EndConversation, NPCSummon, ResortUpdate
...
def normalize_resort_update(state: GameState, update: ResortUpdate) -> ResortUpdate:
    """Repair near-miss NPC ids, then make implicit conversation exits explicit."""
    update = _resolve_npc_ids(state, update)
    current_locations = {
        heartbreaker.id: heartbreaker.location_id for heartbreaker in state.heartbreakers if not heartbreaker.eliminated
    }
...
def _resolve_npc_ids(state: GameState, update: ResortUpdate) -> ResortUpdate:
    ...
    slips: a display name or wrong casing (``Sophie`` -> ``sophie``) and Flush
    Amor heart_throbs named by first name only (``sam`` -> ``sam_ht``). A single
    ...
    known_ids = {heartbreaker.id for heartbreaker in active}
...
def _canonical_npc_id(token: str, active: list[HeartbreakerState], known_ids: set[str]) -> str:
    ...
    if not lowered:
        return token
    for heartbreaker in active:
        if heartbreaker.id.lower() == lowered:
            return heartbreaker.id
    for heartbreaker in active:
        if heartbreaker.name.lower() == lowered:
            return heartbreaker.id
    for heartbreaker in active:
        if heartbreaker.id.lower().split("_", 1)[0] == lowered:
            return heartbreaker.id
    for heartbreaker in active:
        if heartbreaker.name.lower().split(" ", 1)[0] == lowered:
            return heartbreaker.id
    return token


def validate_resort_update(state: GameState, update: ResortUpdate) -> None:
    if state.pending_gather is not None and any(
        ...
        raise ValueError("resort autonomy is paused while a gather is pending")
    known = {heartbreaker.id for heartbreaker in state.heartbreakers if not heartbreaker.eliminated}
    ...
    if len(movement_ids) != len(set(movement_ids)):
        raise ValueError("duplicate NPC movement in ResortUpdate")
    for movement in update.npc_movements:
        _ensure_known_npc(movement.npc_id, known)
        if movement.target_location not in locations_for_resort(state.resort):
            raise ValueError("NPC movement crosses out of the current resort")
    ...
    for start in update.conversation_starts:
        ...
        if start.location not in locations_for_resort(state.resort):
            raise ValueError("NPC conversation start crosses out of the current resort")
        for participant in start.participants:
            _ensure_known_npc(participant, known)
            if location_resort(projected_locations[participant]) is not state.resort:
                raise ValueError(f"NPC conversation participant is not in current resort: {participant}")
...
def _validate_summon(
    ...
    if summon.npc_id in moved_ids:
        raise ValueError("cannot summon and move the same NPC in one ResortUpdate")
    if summon.target_location not in locations_for_resort(state.resort):
        raise ValueError("NPC summon crosses out of the current resort")
    if summon.from_conversation_id == "player_active":
        ...
        if _heartbreaker_location(state, summon.npc_id) != state.location_id:
    ...
    if _heartbreaker_location(state, summon.npc_id) != conversation.location_id:
...
def _heartbreaker_location(state: GameState, heartbreaker_id: str) -> Location:
    for heartbreaker in state.heartbreakers:
        if heartbreaker.id == heartbreaker_id and not heartbreaker.eliminated:
            return heartbreaker.location_id
    raise ValueError(f"unknown active heartbreaker: {heartbreaker_id}")


def _ensure_known_npc(npc_id: str, known: set[str]) -> None:
    if npc_id == "player":
        raise ValueError("player cannot appear in NPC-NPC resort update")
    if npc_id not in known:
        raise ValueError(f"unknown or eliminated NPC in ResortUpdate: {npc_id}")
