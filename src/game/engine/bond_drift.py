"""Overnight bond drift: the resort keeps moving between the player's scenes.
...
- **Consolidation** — an heartbreaker coupled with the player warms a touch: the
...
- **Roving eye** — an heartbreaker who chose *someone else* cools on the player
...
class BondDrift:
    """The realized overnight change to one heartbreaker's bonds with the player.
...
def _partner_id(state: GameState, heartbreaker_id: str) -> str | None:
    """The id of whoever ``heartbreaker_id`` is currently coupled with, or None."""
    for couple in state.couples:
        if couple.partner_a_id == heartbreaker_id:
            return couple.partner_b_id
        if couple.partner_b_id == heartbreaker_id:
            return couple.partner_a_id
    return None


def plan_drift_for(state: GameState, npc: HeartbreakerState) -> BondDrift:
    """Pure: the realized overnight drift for one heartbreaker, given couple state."""
...
def apply_overnight_drift(state: GameState) -> list[BondDrift]:
    """Apply one night of drift to every live heartbreaker; return what actually moved.

    Called once per night by the phase clock at the day rollover. Skips the
    player and eliminated heartbreakers. Deterministic — a pure function of the
