...
between a few player phrasings so repeated demo play does not feel identical,
while the NPC reply stays fixed per (category, outcome) so traces remain
detectable as mock. Determinism (same state -> same line) is required for
replay parity, so the only entropy is the already-deterministic dice roll.
...
class _Line:
    ...
    player: tuple[str, ...]  # phrasings containing a single {name} slot
    npc: str
    tone: Tone
    mood: Mood
...
_LINES: dict[str, dict[str, _Line]] = {
    "friendly": {
        "success": _Line(
            ...
            npc="Aw, stop — I was hoping you'd come over. This is nice.",
            tone="warm",
            mood=Mood.HAPPY,
        ),
        "miss": _Line(
            ...
            npc="Yeah, it's good. *glances away* Sorry — my head's a bit all over the place today.",
            tone="defensive",
            mood=Mood.CONTENT,
        ),
    },
    "flirty": {
        "success": _Line(
            ...
            npc="*bites lip* Smooth. Keep talking like that and you'll turn my head.",
            tone="flirty",
            mood=Mood.FLIRTY,
        ),
        "miss": _Line(
            ...
            npc="*laughs* Easy, tiger. I don't melt quite that fast.",
            tone="amused",
            mood=Mood.CONTENT,
        ),
    },
    "deep": {
        "success": _Line(
            ...
            npc="*softens* That means a lot. Honestly... I don't let many people in either.",
            tone="vulnerable",
            mood=Mood.HAPPY,
        ),
        "miss": _Line(
            ...
            npc="*pauses* That's a lot, this early. Can we get there slowly?",
            tone="defensive",
            mood=Mood.ANXIOUS,
        ),
    },
    "banter": {
        "success": _Line(
            ...
            npc="*cackles* Oi! The cheek of it. I like you already.",
            tone="playful",
            mood=Mood.HAPPY,
        ),
        "miss": _Line(
            ...
            npc="*rolls eyes* Bold words. Bit quieter than I expected, though.",
            tone="amused",
            mood=Mood.CONTENT,
        ),
    },
    "supportive": {
        "success": _Line(
            ...
            npc="*exhales* I really needed to hear that. Thank you.",
            tone="warm",
            mood=Mood.HAPPY,
        ),
        "miss": _Line(
            ...
            npc="I'm fine. *small shrug* Just... not ready to get into it.",
            tone="defensive",
            mood=Mood.ANXIOUS,
        ),
    },
    "gossip": {
        "success": _Line(
            ...
            npc="*gasps* No. Right, tell me everything.",
            tone="amused",
            mood=Mood.HAPPY,
        ),
        "miss": _Line(
            ...
            npc="*frowns* I don't really want to be in the middle of that, if I'm honest.",
            tone="suspicious",
            mood=Mood.ANXIOUS,
        ),
    },
    "bromance": {
        "success": _Line(
            ...
            npc="Likewise, mate. Us against this madhouse, yeah?",
            tone="warm",
            mood=Mood.HAPPY,
        ),
        "miss": _Line(
            ...
            npc="Maybe. *shrugs* Still working you out, to be fair.",
            tone="amused",
            mood=Mood.CONTENT,
        ),
    },
    "gossip_ring": {
        "success": _Line(
            ...
            npc="*grins* Oh, you are SO not. Sit down, we're talking.",
            tone="amused",
            mood=Mood.HAPPY,
        ),
        "miss": _Line(
            ...
            npc="*hesitates* I just don't want this getting twisted, yeah?",
            tone="suspicious",
            mood=Mood.CONTENT,
        ),
    },
...
_DEFAULT: dict[str, _Line] = {
    "success": _Line(
        ...
        npc="*smiles* I'm really glad you came over. This is nice.",
        tone="warm",
        mood=Mood.HAPPY,
    ),
    "miss": _Line(
        ...
        npc="*nods* Yeah... give me a moment, my head's elsewhere right now.",
        tone="defensive",
        mood=Mood.CONTENT,
    ),
}
...
def mock_exchange_fields(
    *,
    ...
    ``roll`` is the deterministic dice roll for the action; it rotates the
    player phrasing so repeated demo play varies without breaking replays.
    """
    outcome = "success" if success else "miss"
    line = _LINES.get(category or "", _DEFAULT).get(outcome, _DEFAULT[outcome])
    index = (roll or 0) % len(line.player)
    player = line.player[index].format(name=target_name)
    return player, line.npc, line.tone, line.mood


# Fixed NPC replies, exposed so report tooling can flag a trace as mock without
# importing the heavyweight agent module. Player lines carry a formatted name so
# they are not stable; NPC lines are constant per (category, outcome).
