...
export function scenarioSummary(scenario: EvalScenario) {
  ...
  if (scenario.judge?.result === "fail") return `All ${checks.length} deterministic checks passed; the thread judge failed.`;
  if (scenario.judge?.result === "cannot_determine") return `All ${checks.length} deterministic checks passed; the thread judge needs review.`;
